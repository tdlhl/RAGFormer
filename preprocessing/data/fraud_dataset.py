...
class FraudDataset(DGLBuiltinDataset):
    ...
    file_urls = {
        ...
        'mimic' : 'dataset/Mimic.zip',
        'chinapay': 'dataset/chinapay.zip',
        'cno': 'dataset/cno.zip'
    }
    relations = {
        'yelp': ['net_rsr', 'net_rtr', 'net_rur'],
        'amazon': ['net_upu', 'net_usu', 'net_uvu'],
        'mimic' : ['rel_vmv', 'rel_vav', 'rel_vpv', 'rel_vdv'],
        'chinapay': ['net_umu', 'net_u2u', 'net_ubu'],
        'cno': ['net_umu', 'net_u2u', 'net_unu']
    }
    file_names = {
        'yelp': 'YelpChi.mat',
        'amazon': 'Amazon.mat',
        'mimic': 'Mimic.mat',
        'chinapay': 'chinapay_normalized.mat',
        'cno': 'cno.mat'
    }
    node_name = {
        'yelp': 'review',
    ...
    def __init__(self, name, raw_dir=None, random_seed=717, train_size=0.7,
        ...
        assert name in ['yelp', 'amazon', 'mimic', 'chinapay', 'cno'], "only supports 'yelp', 'amazon', 'mimic', 'chinapay'"
        url = _get_dgl_url(self.file_urls[name])
        self.seed = random_seed
        self.train_size = train_size
        self.val_size = val_size

        if raw_dir is not None and os.path.exists(os.path.join(raw_dir, self.file_names[name])):
            url = None
    ...
    def download(self):
        ...
        if self.url is None:
            if self.verbose:
                print("Using existing dataset in {}".format(self.raw_dir))
        else:
            # 原有的下载逻辑
            # ... 省略 ...
            pass
...
class FraudMimicDataset(FraudDataset):
    r""" Fraud Mimic Dataset
    ...
    def __init__(self, raw_dir=None, random_seed=717, train_size=0.7,
                 val_size=0.1, force_reload=False, verbose=True):
        super(FraudMimicDataset, self).__init__(name='mimic',
                                                raw_dir=raw_dir,
                                                random_seed=random_seed,
                                                train_size=train_size,
                                                val_size=val_size,
...
class FraudchinapayDataset(FraudDataset):
    r""" Fraud chinapay Dataset

    1. U-M-U: it connects users with same mertype.
    2. U-2-U: it connects users with same M2 rating.
    3. U-B-U: it connects users with same UPPB005(Credit Card Repayment Risk Score) rating.
    ...
    - Classes:
        ...
        - Positive (fraudulent): 22,891
        - Negative (benign): 41,356

    - Positive-Negative ratio: 1 : 1.8
    - Node feature size: 147
    ...
    def __init__(self, raw_dir=None, random_seed=717, train_size=0.7,
                 val_size=0.1, force_reload=False, verbose=True):
        super(FraudchinapayDataset, self).__init__(name='chinapay',
                                                raw_dir=raw_dir,
                                                random_seed=random_seed,
                                                train_size=train_size,
                                                val_size=val_size,
...
class FraudcnoDataset(FraudDataset):
    r""" Fraud chinapay Dataset
    ...
    def __init__(self, raw_dir=None, random_seed=717, train_size=0.7,
                 val_size=0.1, force_reload=False, verbose=True):
        super(FraudcnoDataset, self).__init__(name='cno',
                                                raw_dir=raw_dir,
                                                random_seed=random_seed,
                                                train_size=train_size,
                                                val_size=val_size,
